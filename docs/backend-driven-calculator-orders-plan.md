...
**✅ Slice B completed 2026-06-25** (first batch — arx plain-string — pushed as `6427f5b`; the rest below is uncommitted):
- **dizayn wizard** — partition_material + air_conditioning now schema-driven via `_schemaChipPicker` (`dizayn_tz`); object_type + design_type labels from schema. `dart analyze` clean.
- **enum pickers** (arx object_type/construction_type, dizayn object_type/design_type) — labels driven from the schema via `_schemaEnumLabel(mapsTo, apiValue, fallback)`. Option set + enum storage unchanged (the lists are structural / pricing-bound).
- **`land_use_purpose` freeze-fix** — the wizard now stores the **code** (not the localized label), still shows the label on the chip, and translates the code back in the review. Backend seed field changed `text` → `single_choice` (4 options: turar_joy/ishlab_chiqarish/savdo/aralash). Resolver logic test passes (arxitektura_tz now 15 choice fields).
- **⚠ Deploy note:** the `land_use_purpose` change spans mobile + backend seed — ship the mobile release **together with** a prod `python -m scripts.seed_forms` re-seed, else admin shows the raw `turar_joy` code (not the label) for new orders until re-seeded. (Old orders that stored a label still display fine — resolver passes unknown values through.)
- All touched files `dart analyze` clean; backend `py_compile` + resolver test green. Full `flutter analyze` + on-device pass still recommended before commit.
